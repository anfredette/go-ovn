...
func TestPortGroupACLs(t *testing.T) {
	...
	portGroupACLTests := []ACL{
		...
		{"", "drop", "to-lport", MATCH, 1001, true, []string{"meter1"}, "alert", map[interface{}]interface{}{"A": "a", "B": "b"}},
	}
	...
	t.Run("delete ACLS from non-existent port group", func(t *testing.T) {
		for _, tc := range portGroupACLTests {
			cmd, err = ovndbapi.ACLDelEntity(PORT_GROUP, PG_TEST_NE_PG, tc.Direction, tc.Match, tc.Priority, iMapToSMap(tc.ExternalID))
			assert.NotNil(t, err)
		}
	})
